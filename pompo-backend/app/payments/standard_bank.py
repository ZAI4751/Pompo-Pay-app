"""Standard Bank Malawi adapter.

No Standard Bank Malawi HTTP contract is in the repository. This adapter
encapsulates the *absence* of a live acquiring rail so PaymentService,
webhooks, and instruments fail closed without inventing N-Genius URLs,
payloads, or signatures.

See docs/providers/standard-bank-malawi.md. This module never logs secrets,
PAN, or CVV.
"""
...
COUNTRY = "MW"
CURRENCY = "MWK"
CONTRACT_SOURCE = "docs/providers/standard-bank-malawi.md"
CONTRACT_MISSING = (
    "Standard Bank Malawi live HTTP contract is not in the repository. "
    "See docs/providers/standard-bank-malawi.md."
)

STANDARD_BANK_CAPABILITIES = ProviderCapabilities(
...
class StandardBankMalawiAdapter:
    """Named Standard Bank rail. Never mutates transactions. Never calls invented hosts."""

    code = PROVIDER_CODE
    capabilities = STANDARD_BANK_CAPABILITIES
    ...
    async def verify_webhook(
        ...
        *,
        ...
        logger.warning(
            "standard_bank_webhook_rejected",
            provider=PROVIDER_CODE,
            reason="callback_contract_missing",
        )
        return WebhookVerificationResult(
            False,
            failure_reason="Standard Bank Malawi callback contract is not documented",
        )
    ...
    async def parse_webhook(
        ...
        *,
        ...
        del headers, body
        raise ValueError("Standard Bank Malawi callback payload is not documented")
