"""Master Admin payment-provider management endpoints."""
...
@router.get("", response_model=list[ProviderCatalogResponse])
...
@router.post("", response_model=ProviderCatalogResponse, status_code=status.HTTP_201_CREATED)
async def create_provider(
    ...
    try:
        provider = await service.create_catalog_entry(current_user, payload.model_dump())
    except ProviderCatalogError as exc:
        raise _catalog_error(exc) from exc
    return await catalog_response(provider, service)


@router.get("/{provider_code}", response_model=ProviderCatalogResponse)
...
@router.patch("/{provider_code}", response_model=ProviderCatalogResponse)
...
@router.post("/{provider_code}/enable", response_model=ProviderCatalogResponse)
async def enable_provider(
    provider_code: str, current_user: CurrentUserDep, service: ProviderCatalogServiceDep
) -> ProviderCatalogResponse:
    try:
        provider = await service.enable_provider(current_user, provider_code)
    except ProviderCatalogError as exc:
        raise _catalog_error(exc) from exc
    return await catalog_response(provider, service)


@router.post("/{provider_code}/disable", response_model=ProviderCatalogResponse)
async def disable_provider(
    provider_code: str, current_user: CurrentUserDep, service: ProviderCatalogServiceDep
) -> ProviderCatalogResponse:
    try:
        provider = await service.disable_provider(current_user, provider_code)
    except ProviderCatalogError as exc:
        raise _catalog_error(exc) from exc
    return await catalog_response(provider, service)


@router.get("/{provider_code}/health", response_model=ProviderCatalogResponse)
async def provider_health(
    provider_code: str, current_user: CurrentUserDep, service: ProviderCatalogServiceDep
) -> ProviderCatalogResponse:
    try:
        provider, _health = await service.inspect_health(current_user, provider_code)
    except ProviderCatalogError as exc:
        raise _catalog_error(exc) from exc
    return await catalog_response(provider, service)
